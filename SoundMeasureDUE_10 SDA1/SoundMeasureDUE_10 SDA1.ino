...
const uint64_t pipes[2] = { 0xABCDABCD81LL, 0x544d52688CLL };   // Radio pipe addresses for the 2 nodes to communicate.
...
void synhro_by_main()
{
	...
	while (1)
	{
		...
		if (start_synhro)                                         // Синхроимпульс получен
		{
			...
			{
				if (micros() - StartMeasure > 1500000)                                    // Ожидаем синхроимпульс в течении 1.5 секунд
				{
...
void synhro_by_timer()
{
	...
	while (1)
	{
		...
		if (start_synhro)                                                                  // Синхроимпульс получен
		{
			...
			while (!trig_sin)                                                              // Ожидание времени начала синхроимпульса.  Сигнал формируется таймером Timer5
			{
				if (micros() - StartMeasure > 150000)                                     // Ожидаем синхроимпульс в течении 1.5 секунд
				{
					break;                                                                 // Завершить ожидание синхроимпульса
				}
				for (xpos = 0; xpos < 240; xpos++)                                         // Блоки по 240 байтов
