...
void synhro_by_radio()                                     // просмотр в реальном времени. Синхронизация по радиоимпульсу
{
	...
	while (1)                                            //                
	{
		...
		/*															// Программа чтения данных АЦП. Записать аналоговый сигнал в блок памяти
			...
			myGLCD.printNumI(page_trig, 300, 177);                                   // Вывести на экран страницу срабатывания триггера порога
			myGLCD.setColor(0, 0, 0);
			myGLCD.fillRoundRect(0, 0, 242, 176);                                    // Очистить область экрана 
			DrawGrid1();
			myGLCD.setBackColor(0, 0, 255);                                          // Фон экрана синий
			myGLCD.setColor(255, 255, 255);                                          // Цвет текста белый
			myGLCD.print("        ", 255, 160);                                      // Очистить область вывода информации 
		...
		if (myTouch.dataAvailable())
		{
			...
			if ((x_osc >= 245) && (x_osc <= 318))                  // Боковые кнопки
			{
				...
				{
					waitForIt(245, 135, 318, 175);
					i2c_eeprom_ulong_write(adr_set_timePeriod_RF, EndSample - StartSample);       // Время коррекции задержки на выполнение программы (опция синхронизация по радио )
				}
			}
...
void oscilloscope_file_old()  // Пишет в файл
{
	uint32_t bgnBlock, endBlock;
	block_t block[BUFFER_BLOCK_COUNT];
	myGLCD.clrScr();
...
void wiev_synhro()
{
	...
	myGLCD.printNumI(hh, 280, 25,2);                      // Вывести на экран время час
	myGLCD.print("M:", 250, 40);                          //  
	myGLCD.printNumI(mm, 280, 40,2);                      // Вывести на экран время мин
	myGLCD.print("S:", 250, 55);                          //  
	myGLCD.printNumI(ss, 280, 55,2);                      // Вывести на экран время сек

	unsigned long Start_unixtime = rtc_clock.unixtime();
	...
	//	StartSample = micros();
		...
		if (start_enable)
		{
			...
			myGLCD.printNumI(hh, 280, 95, 2);                      // Вывести на экран время час
			myGLCD.print("M:", 250, 110);                          //  
			myGLCD.printNumI(mm, 280, 110, 2);                     // Вывести на экран время мин
			myGLCD.print("S:", 250, 125);                          //  
			myGLCD.printNumI(ss, 280, 125, 2);                     // Вывести на экран время сек
			myGLCD.print("Duration", 250, 145);                          //  
			myGLCD.print("of time", 253 , 160);

			Current_unixtime = rtc_clock.unixtime();
	...
	if (!sd.begin(SD_CS_PIN, SPI_FULL_SPEED)) 
	  {
		...
		myGLCD.print("\x89""po""\x96\xA0""e""\xA1""a c SD", CENTER, 40);    //  Проблема с SD
		myGLCD.print("\x9F""ap""\xA4""o""\x9E""?", CENTER, 70);    //  картой?
		myGLCD.print("\xA2""e ""\xA5""op""\xA1""a""\xA4\x9D""po""\x97""a""\xA2""a ? ",CENTER, 100);   //  не форматирована?
		myGLCD.print("\xA2""e yc""\xA4""a""\xA2""o""\x97\xA0""e""\xA2""a ? ", CENTER, 130);          //  не установлена? 
		myGLCD.setColor(VGA_LIME);
		myGLCD.print(txt_info11,CENTER, 200);
		myGLCD.setColor(255, 255, 255);
		SD_enable = false;
