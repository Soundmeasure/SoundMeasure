...
void UTFT::SPI_Flash_init(int CS_pin, int Rate)
{
...
void UTFT::Put_Text_array(char *st, int x, int y, int font_number)
{
				...
				{
					case SSD1298:
					LCD_Write_COM_DATA(0x11,0x6030);
					break;
					case R61581_16:
					LCD_Write_COM(0x36); 
					LCD_Write_DATA(0x00);	
				...
				font_address=(unsigned long) font_number * 4096;
						...
						digitalWrite(CSpin,LOW);
 						spiSend(0x03);
 						spiSend(font_address>>16);
 						spiSend(font_address>>8);
 						spiSend(font_address);
 						font_height=(spiRec());
						...
						digitalWrite(CSpin,LOW);
 						...
 						spiSend(font_address>>8);
						if (font_height>70)
						spiSend(font_address+150);
						else 
 						spiSend(font_address+35);
 						font_size=(spiRec());
...
void UTFT::Draw_character(unsigned char character)
{
	...
	if ((character>=0x20)&&(character<=0xFF))
	{
		location = (unsigned long)(character-0x20)*(unsigned long)font_size + (unsigned long)font_address;
					...
					spiSend(location>>8);
					spiSend(location);
					font_header[0] = spiRec();
					font_header[1] = spiRec();
					digitalWrite(CSpin,HIGH);
			//Serial1.write(font_header[0]);		
			//Serial1.write(font_header[1]);
			cbi_CS();
						switch(display_model)
						{
							case SSD1298:
							...
							break;
							case R61581_16:
							setXY_3(PositionX,PositionY,PositionX+font_header[1]+Charspace,PositionY+font_height-1);
							break;	
							case SSD1963_800_2:
							setXY(PositionX,PositionY,PositionX+font_header[1]+Charspace,PositionY+font_height-1);
							break;	
							case ILI9327_2:  		
							setXY_2(PositionX,PositionY,PositionX+font_header[1]+Charspace,PositionY+font_height-1);
			...
			for(i=2;i<temp;i++) 
				   {
					  volatile unsigned char m=spiRec();
					  for(j=0;j<8;j++) 
					  {
						if((m&0x01)==0x01) 	LCD_Write_DATA(ch,cl); 
						else LCD_Write_DATA(ch2,cl2);
						//delay(10);
						m>>=1; 
					 }
					}
			...
			{
				for(unsigned char a=0;a<font_height;a++) {LCD_Write_DATA(ch2,cl2);}
			}
			PositionX += font_header[1]+Charspace;
			digitalWrite(CSpin,HIGH);
			sbi_CS();
		}
		else {return;}
		...
		for (i=0; i<(disp_y_size+1); i++)
			{
				for (j=0; j<(disp_x_size+1); j++)
				{			
//					LCD_Write_DATA(0b11111111,0b11111111);
//					LCD_Write_DATA(0b00000000,0b00000000);					
					 				  if(i>700)Convertto8bit(0xFF,0xFF);
 	 				  else if(i>600)Convertto8bit(0x00,0x1F);
 	 				  else if(i>500)Convertto8bit(0x07,0xE0);
 	 				  else if(i>400)Convertto8bit(0x7F,0xFF);
 	 				  else if(i>300)Convertto8bit(0xF8,0x00);
 	 				  else if(i>200)Convertto8bit(0xF8,0x1F);
 	 				  else if(i>100)Convertto8bit(0xFF,0xE0);
					else Convertto8bit(213,156);	
...
void UTFT::Convertto8bit(char VH, char VL)
{
			char R,G,B;
			LCD_Write_DATA(0,VH&0b11111000);
			LCD_Write_DATA((VH<<5)+(VL>>3),(VL<<3)&0b11111000);
}
...
// 				h=240;
 				cbi_CS();
  				...
  				{
  				case SSD1298:
					LCD_Write_COM_DATA(0x11,0x6038);
					setXY_2(X,Y,X+w-1,Y+h-1);
 					break;
  				case R61581_16:
  					setXY_2(X,Y,X+w-1,Y+h-1);
  					break;
				...
				for (x_1 = 0; x_1<w ; x_1++)
				{
					for (y_1 = 0; y_1<h ; y_1++)
					{
						pixelH=spiRec();
						pixelL=spiRec(); 
						LCD_Writ_Bus(pixelH,pixelL,16);
						//pixelH=spiRec(); 
						//Serial1.write(pixelH);
						//pixelL=spiRec(); 
						//Serial1.write(pixelL);
						//LCD_Write_DATA(pixelH,pixelL); 
						//delay(20);
				}
				}
...
void UTFT::Swtich_LCD_TB_Bit(int TB_Bit)
{
	...
	switch(display_model)
	{
		...
		if (TB_Bit==1)
		{
			LCD_Write_COM_DATA(0x01,0x3B3F); //0x2B3F  for original testing
		}		
		else
		{
			LCD_Write_COM_DATA(0x01,0x393F); //0x2B3F for custom font testing
		}	
		break;
		case R61581_16:
		if (TB_Bit==1)
...
// 		LCD_Write_DATA(0x00,0x02);
    ...
    LCD_Write_DATA(0x0000);
		...
		LCD_Write_COM(0xB4);		//HSYNC
	LCD_Write_DATA((928>>8)&0X00FF);  //Set HT
	LCD_Write_DATA(928&0X00FF);
	LCD_Write_DATA((46>>8)&0X00FF);  //Set HPS
	LCD_Write_DATA(46&0X00FF);
	LCD_Write_DATA(48);			   //Set HPW
	LCD_Write_DATA((15>>8)&0X00FF);  //Set HPS
	LCD_Write_DATA(15&0X00FF);
	LCD_Write_DATA(0x0000);

		LCD_Write_COM(0xB6);		//VSYNC
	LCD_Write_DATA((525>>8)&0X00FF);   //Set VT
	LCD_Write_DATA(525&0X00FF);
	LCD_Write_DATA((16>>8)&0X00FF);  //Set VPS
	LCD_Write_DATA(16&0X00FF);
	LCD_Write_DATA(16);			   //Set VPW
	LCD_Write_DATA((8>>8)&0X00FF);  //Set FPS
	LCD_Write_DATA(8&0X00FF);		
		...
		delay(9);	
		LCD_Write_COM(0xF0);		//pixel data interface 1 2 3
		LCD_Write_DATA(0x09);		
		
	delay(9);	
		LCD_Write_COM(0x26);		//display on
		LCD_Write_DATA(0x08);
...
void UTFT::setXY_3(word x1, word y1, word x2, word y2)
{
	int tmp;
